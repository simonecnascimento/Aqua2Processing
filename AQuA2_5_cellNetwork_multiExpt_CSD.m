...
for experiment = 1:length(sortedFileNames)
    ...
    baseline_preCSD_frames = [927;1854];
    
    % separate events in pre, during and postCSD
    [eventsByCell_experiment, frameNumbers, indexes] = eventsCSDphases(data_analysis, preCSD_frames, duringCSD_frames, postCSD_frames, baseline_preCSD_frames);
    eventsByCell_all = [eventsByCell_all; eventsByCell_experiment];

    % Calculate the total number of events by phase
    totalEvents_preCSD = sum(cellfun(@numel, eventsByCell_experiment(:,2)));
    totalEvents_duringCSD = sum(cellfun(@numel, eventsByCell_experiment(:,3)));
    totalEvents_postCSD = sum(cellfun(@numel, eventsByCell_experiment(:,4)));
    totalEvents_baseline_preCSD = sum(cellfun(@numel, eventsByCell_experiment(:,5)));
    ...
    % preCSD
    networkData_preCSD = [propagationMap_event, cellNumber_event, cellType_event, cellMap_event, nSimultanousEvents, ...
    ...
    networkData_preCSD = cell2table(networkData_preCSD);
    networkData_preCSD.Properties.VariableNames = {'propagationMap_event','cellNumber_event', 'cellType_event','cellMap_event', 'nSimultanousEvents', ...
    ...
    % duringCSD
    networkData_duringCSD = [propagationMap_event, cellNumber_event, cellType_event, cellMap_event, nSimultanousEvents, ...
        simultaneousEvents_all, propagationMap_all, cellNumber_all, cellType_all, cellMap_all, ...
        cellNumber_all_unique, cellMap_all_unique, eventNetwork_cellMap_all, shortestDistanceBetweenCenters, ...
        simultaneousEvents_network, simultaneousEvents_network_corrected, propagationMap_network, cellNumber_network, cellType_network, cellMap_network];
    networkData_duringCSD = cell2table(networkData_duringCSD);
    networkData_duringCSD.Properties.VariableNames = {'propagationMap_event','cellNumber_event', 'cellType_event','cellMap_event', 'nSimultanousEvents', ...
        'simultaneousEvents_all', 'propagationMap_all', 'cellNumber_all', 'cellType_all', 'cellMap_all', 'cellNumber_all_unique', 'cellMap_all_unique', 'eventNetwork_cellMap_all', 'shortestDistanceBetweenCenters', ...
        'simultaneousEvents_network', 'simultaneousEvents_network_corrected', 'propagationMap_network', 'cellNumber_network', 'cellType_network', 'cellMap_network'};

    % postCSD
    networkData_postCSD = [propagationMap_event, cellNumber_event, cellType_event, cellMap_event, nSimultanousEvents, ...
        simultaneousEvents_all, propagationMap_all, cellNumber_all, cellType_all, cellMap_all, ...
        cellNumber_all_unique, cellMap_all_unique, eventNetwork_cellMap_all, shortestDistanceBetweenCenters, ...
        simultaneousEvents_network, simultaneousEvents_network_corrected, propagationMap_network, cellNumber_network, cellType_network, cellMap_network];
    networkData_postCSD = cell2table(networkData_postCSD);
    networkData_postCSD.Properties.VariableNames = {'propagationMap_event','cellNumber_event', 'cellType_event','cellMap_event', 'nSimultanousEvents', ...
        'simultaneousEvents_all', 'propagationMap_all', 'cellNumber_all', 'cellType_all', 'cellMap_all', 'cellNumber_all_unique', 'cellMap_all_unique', 'eventNetwork_cellMap_all', 'shortestDistanceBetweenCenters', ...
        'simultaneousEvents_network', 'simultaneousEvents_network_corrected', 'propagationMap_network', 'cellNumber_network', 'cellType_network', 'cellMap_network'};
...
% %     cellMaps = networkData_cleaned{currentEvent, 'cellMap_all'}{1}; % cellMap_all might already be a cell array
    ...
    preCSD_events = cell2mat(eventsByCell_experiment(:,2));
    duringCSD_events = cell2mat(eventsByCell_experiment(:,3));
    postCSD_events = cell2mat(eventsByCell_experiment(:,4));

    % Define the three sets of networkData
    networkDataSets = {'networkData_preCSD', 'networkData_duringCSD', 'networkData_postCSD'};
    eventsSets = {preCSD_events, duringCSD_events, postCSD_events};  % Corresponding events for each set

    % Variable names for final assignment
    tableNames = {'networkData_preCSD', 'networkData_duringCSD', 'networkData_postCSD'};
    ...
    for phase = 1:3
        ...
        for currentEvent = 1:numEvents
    
            if ismember(currentEvent, phaseEvents)
                ...
                for a = 1:length(simultaneousEvents_current)
                    ...
                    if ~ismember(simultaneousEvent, phaseEvents)
                        simultaneousEvent = [];
...
%         networkData_duringCSD = networkDataSets{2};
%         networkData_postCSD = networkDataSets{3};
    ...
    %             title('Combined Non-Zero Values from All Cell Maps');
        ...
        % Digraph
        [adjMatrix, centers_allCells, G, rowsWithSingleAppearance, nodeDegree] = plotCellDistanceNetwork(data_CFU, data_analysis, simultaneousMatrixDelaybyCell, simultaneousMatrixDelaybyCell_average, pwd, AquA_fileName, tableNames, phase);
        ...
        resultantVector = sumEdgeVectors(G, centers_allCells, rowsWithSingleAppearance, pwd, AquA_fileName, tableNames, phase);
    
        % Identify connected networks
        saveConnectedNetworks(G, centers_allCells, pwd, AquA_fileName, tableNames, phase);
        ...
        networkFilename = fullfile(subfolderNetworkPath, strcat(fileTemp, '_', tableNames{phase}, '_network_propagation.mat'));
        save(networkFilename, '-v7.3');
    end
end

% AFTER COMPUTING ALL EXPERIMENTS
