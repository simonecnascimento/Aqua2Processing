function [directions, directionCounts, directionLabels, clusterID, rates_mHz, rates_mHz_clusterID] = clusterWaves_eventRate6_CSD(eventHz_byCell, cellTypes)
...
%     rates_uHz  - Nx3 matrix of event rates in µHz [pre during post]
    ...
    tol = 1e-4;  % or 1e-3 if your data has less precision

    % PRE-TO-DURING Logical conditions for classification of events 
    %median_preCSD = median(rates_mHz(:,1));
    %increasingEventsChange1 = rates_mHz(:, 2) >= 16.6667;
    increasingEventsChange1 = rates_mHz(:, 2) >= 2 * rates_mHz(:, 1) - tol & ~(rates_mHz(:, 1) == 0 & rates_mHz(:, 2) == 0);
    noChangeEventsChange1 = ~(increasingEventsChange1);

    % PRE-TO-POST Logical conditions for classification of events 
    increasingEventsChange2 = rates_mHz(:, 3) >= 2 * rates_mHz(:, 1) - tol & ~(rates_mHz(:, 1) == 0 & rates_mHz(:, 3) == 0);
    decreasingEventsChange2 = rates_mHz(:, 3) <= 0.5 * rates_mHz(:, 1) + tol & ~(rates_mHz(:, 1) == 0 & rates_mHz(:, 3) == 0);
    noChangeEventsChange2 = ~(increasingEventsChange2 | decreasingEventsChange2) | (rates_mHz(:, 1) == 0 & rates_mHz(:, 3) == 0);
    ...
    % Create 6 clusters based on change1 and change2
    ...
    for i = 1:length(directionLabels)
        ...
        col = find(directions == directionLabels(i));
